$(document).ready(function(){
			...
			window.textfont = $(".quesPart").css("font-size");
				...
				$('.fsize').css("font-size",textfont*.95+"px");
				$('.size').css("font-size",textfont*.75+"px");
	/*------------------------------------------------------------------------------------------------------------------------*/			
...
if(checkMobile){
	...
	if(w_Set > h_Set)
	{
		...
		$(".A").show();
			...
			if(String(navigator.userAgent).indexOf("Edge") != -1 ){$('.tick,.cross').css("font-size",textfont*.55+"px");}else{$('.tick,.cross').css("font-size",textfont*.75+"px");}
