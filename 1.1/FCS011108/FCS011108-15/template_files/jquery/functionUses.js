...
window.checkMobile=isMobileDevice();
	//alert($(".container-fluid").css("font-size"))
...
///////////////////////////////////////////////////////////////////							   
		...
		NumberToWord_FirstCapital = function(i)  {
			...
			if (i<20) {
				...
				//alert(tens[Math.floor(i/10)])
				 Final_Str = String(tens[Math.floor(i/10)]).charAt(0).toUpperCase() + String(tens[Math.floor(i/10)]).slice(1)+" -"+((i % 10 > 0)?" "+String(units[i % 10]).charAt(0).toLowerCase() + String(units[i % 10]).slice(1):"")
			}
			return Final_Str;
		};
		
		NumberToWord_FirstCapital1 = function(i)  {
			var Final_Str = "";
			var units = new Array();
			units = ["Zero", "One", "Two", "Three", "Four", "Five", "Six", "Seven", "Eight", "Nine", "Ten", "Eleven", "Twelve", "Thirteen", "Fourteen", "Fifteen", "Sixteen", "Seventeen", "Eighteen", "Nineteen"];
			var tens = new Array();
			tens = ["", "", "twenty", "thirty", "forty", "fifty", "sixty", "seventy", "eighty", "ninety"];
			//if (i<20) {
			Final_Str = units[String(i).split("")[0]]+" - "+String(units[String(i).split("")[1]]).toLowerCase();
			//}
			return Final_Str;
		};
		
		NumberToWord_FirstCapital2 = function(i)  {
			var Final_Str = "";
			var units = new Array();
			units = ["Zero", "One", "Two", "Three", "Four", "Five", "Six", "Seven", "Eight", "Nine", "Ten", "Eleven", "Twelve", "Thirteen", "Fourteen", "Fifteen", "Sixteen", "Seventeen", "Eighteen", "Nineteen"];
			var tens = new Array();
			tens = ["", "", "twenty", "thirty", "forty", "fifty", "sixty", "seventy", "eighty", "ninety"];
			//if (i<20) {
			Final_Str = units[String(i).split("")[1]]+" - "+String(units[String(i).split("")[0]]).toLowerCase();
			//}
			return Final_Str;
		};
			
			
		NumberToWord_FirstCapital3 = function(i)  {
			var ReveseNum = String(i).split("").reverse().join("")
			var Final_Str = "";
			var units = new Array();
			units = ["Zero", "One", "Two", "Three", "Four", "Five", "Six", "Seven", "Eight", "Nine", "Ten", "Eleven", "Twelve", "Thirteen", "Fourteen", "Fifteen", "Sixteen", "Seventeen", "Eighteen", "Nineteen"];
			var tens = new Array();
			tens = ["", "", "twenty", "thirty", "forty", "fifty", "sixty", "seventy", "eighty", "ninety"];
			if (ReveseNum<20) {
				Final_Str = units[ReveseNum];
			} else if (ReveseNum<100) {
				 Final_Str = String(tens[Math.floor(ReveseNum/10)]).charAt(0).toUpperCase() + String(tens[Math.floor(ReveseNum/10)]).slice(1)+" -"+((i % 10 > 0)?" "+String(units[ReveseNum % 10]).charAt(0).toLowerCase() + String(units[ReveseNum % 10]).slice(1):"")
			}
			return Final_Str;
		};	
			
	//alert(NumberToWord_FirstCapital4("12"))		
